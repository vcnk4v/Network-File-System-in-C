...
#include "../defs.h"
#define CLIENT_PORT 5555 // Port for client connection
...
void initializeAccessiblePaths()
{
    ...
    strcpy(accessiblePaths[0], "dirb1/file3");
    strcpy(accessiblePaths[1], "dira1/dird1/file2");
    strcpy(accessiblePaths[2], "dira1/file1");
    strcpy(accessiblePaths[3], "dirc1/file4");
    strcpy(accessiblePaths[4], "dira1/dire1/file5");
...
int Handle_Read_Request(char *buffer, int client_connection)
{
    ...
    while (buffer_index < file_size)
    {
        if (buffer_index + CHUNK_SIZE <= file_size)
        {
            ...
            if (send(client_connection, file_contents, sizeof(file_contents), 0) < 0)
            {
